...
export function Bills({ bill }) {
  ...
  function handleDeletionBill (billName) {
    const billRef = ref(database, `bills/${billName}`)
    remove(billRef)
  }
  ...
  return (
    <Popover.Root>
       <Popover.Trigger className={clsx('bg-inputbg  w-48 h-14 rounded-lg transition-colors', {
        ...
        <Popover.Anchor />
        <Popover.Portal>
          <Popover.Content className='flex flex-col mt-4 min-w-[320px] gap-3 bg-inputbg p-4 rounded-lg border-2 border-gpurlpe'>
            <div>
              <ul>
                ...
                <div className='flex justify-center mt-5 gap-3'>
                  ...
                  <button 
                    ...
                    onClick={() => handleDeletionBill(bill.name)}
                  >
